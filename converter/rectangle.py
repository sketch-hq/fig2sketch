    ...
    return Rectangle(
        **base.base_shape(fig_rect),
        ...
        corners=Rectangle.Corners(
            fig_rect.get("rectangleTopLeftCornerRadius", 0)
            if independent
            else corner_radius,
            fig_rect.get("rectangleTopRightCornerRadius", 0)
            if independent
            else corner_radius,
            fig_rect.get("rectangleBottomRightCornerRadius", 0)
            if independent
            else corner_radius,
            fig_rect.get("rectangleBottomLeftCornerRadius", 0)
            if independent
            else corner_radius,
        ),
    )
...
def make_background_rect(guid, frame, name) -> Rectangle:
    if type(frame) == Rectangle:
        ...
        rectangle = Rectangle(
            ...
            style=Style(
                do_objectID=utils.gen_object_id(guid, f"{name}_style".encode())
            ),
